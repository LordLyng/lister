...
export const AppListItem = ({ item, removeItem, editIten, deleteItemsOnChecked }: Props) => {
  return (
    <Flex align="center" gap="md" flex="1 1 auto" opacity={item.checked ? 0.6 : 1}>
      <Checkbox
        onChange={(event) => {
      ...
      />
      <EditableText text={item.text} onEdit={(text) => editIten({...item, text})} />
    </Flex>
  );
};
